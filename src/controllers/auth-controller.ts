...
export default {
    ...
    try {
      ...
      if (isLocked) {
        ...
        return res.status(429).json({
          error:
            "Too many failed login attempts. Account is temporarily locked. Please try again later or reset your password.",
        });
      }
  ...
  passwordUpdate: async (req: URequest, res: Response) => {
    try {
      ...
      return res
        .status(500)
        .json({ error: "An error occurred during password update" });
    }
  },
    ...
    try {
      ...
      if (await authService.isPasswordCompromised(password)) {
        ...
        if (!newPassword) {
          return res
            .status(500)
            .json({ error: "Error generating secure password" });
        }
        ...
        await sendPassword(
          email,
          "P.A.R.K Exams center password reset code",
          newPassword,
          user.firstName,
          user.lastName
        );
  ...
  token: async (req: URequest, res: Response) => {
    try {
      ...
      return res
        .status(500)
        .json({ error: "An error occurred during token generation" });
    }
  },
};
